...
function navigateTo(page) {
    ...
    if (page === 'project-leads') {
        // Show projects page (non-priority by default)
        const projectsPage = document.getElementById('page-projects');
        if (projectsPage) {
            projectsPage.classList.add('active');
        }
        // Update sidebar to show Projects as active
    ...
    } else if (page === 'sales-reps') {
        // Show sales reps page
        const salesRepsPage = document.getElementById('page-sales-reps');
        if (salesRepsPage) {
            salesRepsPage.classList.add('active');
        }
        // Update sidebar
        document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active'));
        const salesRepsNavItem = document.querySelector('.nav-item[data-page="sales-reps"]');
        if (salesRepsNavItem) {
            salesRepsNavItem.classList.add('active');
        }
    } else if (page === 'priority-projects') {
        // Show priority projects page
        const priorityPage = document.getElementById('page-priority-projects');
        if (priorityPage) {
            priorityPage.classList.add('active');
        }
        // Update sidebar
        document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active'));
        const priorityNavItem = document.querySelector('.nav-item[data-page="priority-projects"]');
        if (priorityNavItem) {
            priorityNavItem.classList.add('active');
        }
    } else {
